...
export class TextControl {
     ...
     */
    constructor({ apiKey = TextControl.readEnv("OPENAI_API_KEY"), ...config } = {}) {
    ...
    async sendRequest(message) {
        try {
            ...
            try {
                ...
                console.log("Sending request with fallback model failed, switching to default model, returning");
                this.setDefaultClient();
                throw fallbackError;
            }
        }
     ...
     */
    async getFunctionCallName(message) {
        try {
            const tools = this.functions.map(({ description }) => ({ ...description }));
            const chatCompletion = await this.client.chat.completions.create({
    ...
    addFunction(description, func) {
        ...
        const existingFunctionIndex = this.functions.findIndex((f) => f.description.function.name === validatedDescription.function.name);
