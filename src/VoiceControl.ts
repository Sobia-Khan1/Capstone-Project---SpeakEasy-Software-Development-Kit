...
export class VoiceControl {
	...
	private transcriptionHandler: ((transcript: string) => void) | null = null;

	/**
	 * Registers a transcription handler to process real-time transcriptions.
	 * @param handler Function to handle transcriptions.
	 */
	registerTranscriptionHandler(handler: (transcript: string) => void) {
	...
	addFunction(description: FunctionDefinitionSchema, func: (...args: any[]) => any) {
		...
		const existingFunctionIndex = this.functions.findIndex(
			(f) => f.description.function.name === description.function.name
		);
	 ...
	 */
	handleTranscription(transcript: string) {
	 ...
	 */
	async handleFunctionCall(event: any): Promise<void> {
		if (!event.arguments) {
